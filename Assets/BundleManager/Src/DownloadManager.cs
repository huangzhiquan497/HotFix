...
public class DownloadManager : MonoBehaviour 
{
	 ...
	 */ 
	public bool IsBundleCached(string url)
	{
		if (!ConfigLoaded) 
		{
			Debug.LogError ("Cannot get bundle cache state before initialization finished.");
			return false;
		}

		if (isAbsoluteUrl (url)) 
		{
			Debug.LogWarning ("BM cannot cache files downloaded by absolute url.");
			return false;
		}

		string bundleName = stripBundleSuffix (url);

		if (!bundleDict.ContainsKey (bundleName)) 
		{
			Debug.LogWarning ("BM cannot cache files out of the bundle list.");
			return false;
		}

		BundleBuildState bbs = buildStatesDict [bundleName];
		url = formatUrl(url);
		return Caching.IsVersionCached (url, bbs.version);
	}
	...
	IEnumerator Start() 
	{
		...
		if(bmUrl.offlineCache)
		{
			...
			{
				// Download new config file succeed, update cache version number
		...
		if(initDataWWW.error != null && initDataWWW.error != "")
		{
	...
	void download(WWWRequest request)
	{
		...
		if(processingRequest.ContainsKey(request.url) || succeedRequest.ContainsKey(request.url))
			return;
	...
	{
		// Override priority
		foreach (WWWRequest req in waitingRequests) 
		{
...
//		}
	...
	public static DownloadManager Instance
	{
		get
		{
			...
			{
				IsInitFailed = false;
				instance.StartCoroutine(instance.Start());
			}
	...
	/**
	 * Redirect the root url by set it in runtime code.
	 * This method should only be called before the property Instance is referenced.
	 */ 
	...
	public static bool IsInitFailed{get; private set;}
