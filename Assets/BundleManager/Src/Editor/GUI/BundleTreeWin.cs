...
internal class BundleTreeWin : EditorWindow 
{
	...
	private void UploadBuiltSelection()
	{
...
//			
//			AppPacker.GenerateFileAndUpload(BuildConfiger.UnityBuildTarget, m_Selections.ToList(), bundleServer);
//		}
	}
	...
	void EditProcess(Rect itemRect, string bundleName)
	{
		if(m_CurrentEditing == bundleName)
		{
			...
			bool clickOutSideTheTextField = m_CurrentEditing != "" && Event.current.type == EventType.MouseUp && !IsRectClicked(itemRect);
			bool isFinishedEdit = Event.current.type == EventType.KeyUp && Event.current.keyCode == KeyCode.Return;
			if((!HasFocuse() || clickOutSideTheTextField || isFinishedEdit) && Event.current.type != EventType.Layout)
			{
		...
		else if(IsMouseOn(itemRect) && Event.current.type == EventType.MouseDown && Event.current.button == 0)
		{
			Event.current.Use();
		}
	}
	...
	{
		return Event.current.type == EventType.MouseUp && IsMouseOn(rect);
	}
	...
	void CheckAssetSortValid()
	{
//		BundleAssetsChecker.CheckBundleAssetsSortInvalid(m_Selections.ToList());
	}

	void BuildSelectionAndUpload()
	{
//		AppPacker.BuildAndUpload(BuildConfiger.UnityBuildTarget, m_Selections.ToList());
	}
